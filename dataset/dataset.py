...
def load_multimodal_features_from_h5(feat_h5_video, feat_h5_audio, feature_names_list, 
    ...
    if 'vggish_features' in feature_names_list:
        ...
        audio = os.path.join(feat_h5_audio, video_id)+".npy"
        audio_stack = np.load(open(audio, "rb"))
    ...
    if 'i3d_features' in feature_names_list:
        ...
        rgb = os.path.join(feat_h5_video,video_id)+"_rgb.npy"
        flow = os.path.join(feat_h5_video,video_id)+"_flow.npy"      

        video_stack_rgb = np.load(open(rgb,"rb"))
        video_stack_flow = np.load(open(flow,"rb"))
...
class AudioVideoFeaturesDataset(Dataset):
    ...
    def __init__(self, video_features_path, video_feature_name, 
        ...
        self.feat_h5_video = self.video_features_path
        #self.feat_h5_audio = h5py.File(audio_features_path, 'r')
        self.feat_h5_audio = self.audio_features_path
