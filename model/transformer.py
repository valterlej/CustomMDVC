...
class SubsAudioVideoGeneratorConcatLinearDoutLinear(nn.Module):
    ...
    def __init__(self, d_model_subs, d_model_audio, d_model_video, voc_size, dout_p, model_type):
        ...
        if self.model_type == "visualaudio":
            self.linear = nn.Linear(d_model_audio + d_model_video, voc_size)
            print('using AudioVideoGeneratorConcatLinearDoutLinear')
        else:
            self.linear = nn.Linear(d_model_video, voc_size)
            print('using VideoGeneratorConcatLinearDoutLinear')
        self.dropout = nn.Dropout(dout_p)
...
class SubsAudioVideoTransformer(nn.Module):
    ...
    def __init__(self, trg_voc_size, src_subs_voc_size,
                 ...
                 dout_p, H, use_linear_embedder, model_type):
        ...
        self.generator = SubsAudioVideoGeneratorConcatLinearDoutLinear(
            d_model_subs, d_model_audio, d_model_video, trg_voc_size, dout_p, model_type
        )
    ...
    def forward(self, src, trg, mask):
        ...
        if self.model_type == "visualaudio":
            out = self.generator(None, out_audio, out_video)
        else:
            out = self.generator(None, None, out_video)
